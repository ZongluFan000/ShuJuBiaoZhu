...
def main() -> int:
    ...
    dest = ROOT / "data" / "input" / "rules_representative_20.xlsx"
    ...
    ws = wb.active
    ws.title = "representative_rules"
    headers = ["试验标识", "试验注册号", "试验方案编号", "山东大学齐鲁医院", "癌症类型", "癌症分类", "试验分期", "试验分期", "伦理", "标准编号", "规则", "规则标识"]
    ws.append(headers)
    for rule in selected:
        ws.append([
            rule.trial_id,
            rule.trial_register_id,
            rule.protocol_no,
            "",
            rule.cancer_type,
            rule.cancer_category,
            "",
            "",
            "",
            rule.standard_no,
    ...
    for idx, rule in enumerate(selected, 1):
        print(idx, classify_rule(rule).category, rule.trial_id, rule.standard_no, rule.rule_type, rule.rule_text[:80])
    return 0
