const state = {
  ...
  pairSelections: JSON.parse(localStorage.getItem("annotationPairSelections") || "{}"),
  checkpoints: {items: []},
  tasks: {items: []},
...
const viewMeta = {
  overview: ["V3 运行工作台", "确认 V3 标准层范围、启动任务、查看进度和输出。"],
  patients: ["患者选择", "选择本次要运行的患者文件；任务规则固定为 V3 标准层。"],
  models: ["模型与并发", "V3 默认使用单患者内 12 个规则批次并发。"],
...
function pairSelectionKey(idx) {
  return `${state.runMode}:${idx}`;
}

function selectedPairValue(pair, idx, field) {
  return state.pairSelections[pairSelectionKey(idx)]?.[field] || pair[field];
}

function savePairSelection(idx, field, value) {
  const key = pairSelectionKey(idx);
  state.pairSelections[key] = {...(state.pairSelections[key] || {}), [field]: value};
  localStorage.setItem("annotationPairSelections", JSON.stringify(state.pairSelections));
}
...
function syncActionButtons(running) {
  ...
  ["overviewNewTaskBtn", "newTaskBtn"].forEach(id => {
    if ($(id)) $(id).disabled = Boolean(running);
  });
...
function renderWorkbench() {
  ...
  const stability = progress.server_stability || {};
  const avgWallPatient = Number(progress.avg_wall_seconds_per_patient || 0);
  const avgPurePatient = Number(progress.avg_pure_patient_elapsed_seconds || progress.avg_patient_elapsed_seconds || 0);
  ...
  $("runScope").insertAdjacentHTML("beforeend", `
    <div><b>平均每患者总耗时</b><span>${formatDuration(avgWallPatient)}</span></div>
    <div><b>平均纯患者耗时</b><span>${formatDuration(avgPurePatient)}</span></div>
    <div><b>服务器稳定时间</b><span>${formatDuration(Number(stability.stable_seconds || 0))}</span></div>
    <div><b>服务器不稳定时间</b><span>${formatDuration(Number(stability.unstable_seconds || 0))}</span></div>
    <div><b>服务器稳定率</b><span>${stability.stability_ratio != null ? `${Math.round(Number(stability.stability_ratio || 0) * 10000) / 100}%` : "-"}</span></div>
  `);
...
function progressCard(item) {
  ...
  const stability = item.server_stability || {};
  return `
    <div class="progress-card">
      ...
      <div class="progress-meta">
        <span>总均耗时 ${formatDuration(item.avg_wall_seconds_per_patient)}</span>
        <span>纯均耗时 ${formatDuration(item.avg_pure_patient_elapsed_seconds || item.avg_patient_elapsed_seconds)}</span>
        <span>稳定 ${formatDuration(Number(stability.stable_seconds || 0))}</span>
        <span>不稳定 ${formatDuration(Number(stability.unstable_seconds || 0))}</span>
        <span>稳定率 ${stability.stability_ratio != null ? `${Math.round(Number(stability.stability_ratio || 0) * 10000) / 100}%` : "-"}</span>
      </div>
...
async function loadProjects() {
  ...
  const defaultModel = data.pairs?.[0]?.model_config || data.model_configs?.[0] || "";
  $("benchModelSelect").innerHTML = optionList(data.model_configs || [], defaultModel);
  renderProjectPairs("projectPairs");
  renderProjectPairs("overviewProjectPairs");
...
function renderProjectPairs(containerId) {
  ...
  $(containerId).innerHTML = pairs.map((pair, idx) => `
    <div class="pair-row ${compact ? "compact" : ""}">
      ...
      <select data-pair-project="${idx}">${optionList(projects, selectedPairValue(pair, idx, "project_config"))}</select>
      <select data-pair-model="${idx}">${optionList(models, selectedPairValue(pair, idx, "model_config"))}</select>
...
function collectSelectedPairs(containerId, singleIndex = null) {
  ...
  pairs.forEach((pair, idx) => {
    ...
    savePairSelection(idx, "project_config", projectSelect.value);
    savePairSelection(idx, "model_config", modelSelect.value);
    selected.push({
      name: pair.name,
...
async function loadCheckpoints() {
  const [data, archives] = await Promise.all([
    api("/api/checkpoints"),
    api("/api/checkpoint-archives")
  ]);
  state.checkpoints = data;
  state.checkpointArchives = archives;
  ...
  $("checkpointArchiveList").innerHTML = (archives.items || []).map(renderCheckpointArchive).join("") || "<p class='status'>暂无历史任务归档。</p>";
}
...
function renderCheckpointArchive(item) {
  const pct = Number(item.percent || 0);
  return `
    <div class="checkpoint-card">
      <div class="progress-head">
        <strong>${escapeHtml(item.task_name || item.project_config)}${item.run_id ? ` / ${escapeHtml(item.run_id)}` : ""}</strong>
        <span class="badge">${escapeHtml(item.reason || "archive")}</span>
      </div>
      <div class="progress-bar"><span style="width:${Math.min(100, pct)}%"></span></div>
      <div class="scope-box">
        <div><b>归档时间</b><span>${formatDateTime(item.archived_at)}</span></div>
        <div><b>历史进度</b><span>${pct}% (${item.processed_tasks || 0}/${item.total_tasks || "-"})</span></div>
        <div><b>成功/失败</b><span>${item.done_tasks || 0} / ${item.failed_tasks || 0}</span></div>
        <div><b>患者状态数</b><span>${item.patient_status_rows || 0}</span></div>
        <div><b>文件数量</b><span>${escapeHtml(item.file_count ?? "-")}</span></div>
        ${item.output_dir ? `<div><b>输出目录</b><code>${escapeHtml(item.output_dir)}</code></div>` : ""}
        <div><b>归档路径</b><code>${escapeHtml(item.path)}</code></div>
      </div>
      <button class="danger-btn small" data-delete-archive="${escapeHtml(item.path)}">删除历史归档</button>
    </div>
  `;
}
...
async function loadTasks() {
  ...
  state.tasks = data;
  if ($("taskConsoleUpdated")) {
    $("taskConsoleUpdated").textContent = data.updated_at ? `更新时间：${formatDateTime(data.updated_at)}` : "";
  }
  if ($("taskConsoleList")) {
    $("taskConsoleList").innerHTML = (data.items || []).map(renderTaskCard).join("") || "<p class='status'>暂无任务。</p>";
  }
}
...
function renderTaskCard(item) {
  ...
  const badgeClass = item.active ? "running" : (isCurrent ? "ok" : "");
  const badgeText = item.active ? "运行中" : (isCurrent ? "当前任务" : "历史任务");
  return `
    <div class="task-card ${isCurrent ? "current" : "archive"}">
      <div class="progress-head">
        <strong>${escapeHtml(item.task_name || item.run_id || item.project_config || "未命名任务")}</strong>
        <span class="badge ${badgeClass}">${badgeText}</span>
      </div>
      <div class="progress-bar"><span style="width:${Math.min(100, pct)}%"></span></div>
      <div class="progress-meta">
        <span>${pct}%</span>
        <span>${item.processed_tasks || 0}/${item.total_tasks || "-"}</span>
        <span>成功 ${item.done_tasks || 0}</span>
        <span>失败 ${item.failed_tasks || 0}</span>
      </div>
      <div class="progress-path">${escapeHtml(item.project_config || "")}${item.output_dir ? ` · ${escapeHtml(item.output_dir)}` : ""}</div>
      ${isCurrent ? `
        <div class="task-actions">
          <button class="primary small" data-start-current-task="1">继续运行这个任务</button>
        </div>
      ` : `
        <div class="task-actions">
          <button class="primary small" data-restore-archive="${escapeHtml(item.path)}">恢复并切换到这个任务</button>
          <button class="danger-btn small" data-delete-archive="${escapeHtml(item.path)}">删除历史归档</button>
        </div>
      `}
    </div>
...
async function restoreCheckpointArchive(path) {
  if (activeRuns().length) {
    $("checkpointActionStatus").textContent = "当前仍有任务运行，请先中断任务，再恢复历史任务。";
    return;
  }
  const ok = confirm("确认恢复这个历史任务吗？当前 checkpoint 会先自动归档保存，然后用这个历史 checkpoint 替换当前 checkpoint。");
  if (!ok) return;
  $("checkpointActionStatus").textContent = "正在恢复历史任务 checkpoint...";
  const data = await api("/api/restore-checkpoint", {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify({archive_path: path})
  });
  $("checkpointActionStatus").textContent = `已恢复 ${data.project_config} 的历史任务。现在点击“启动勾选任务”即可从该断点续跑。`;
  if ($("taskConsoleStatus")) $("taskConsoleStatus").textContent = `已切换到 ${data.task_name || data.project_config}，点击“启动勾选任务”即可续跑。`;
  await Promise.all([loadProjects(), loadProgress(), loadCheckpoints(), loadStats(), loadTasks()]);
}

async function deleteCheckpointArchive(path) {
  if (activeRuns().length) {
    if ($("checkpointActionStatus")) $("checkpointActionStatus").textContent = "当前仍有任务运行，请先中断任务，再删除历史归档。";
    if ($("taskConsoleStatus")) $("taskConsoleStatus").textContent = "当前仍有任务运行，请先中断任务，再删除历史归档。";
    return;
  }
  const ok = confirm("确认删除这个历史任务归档吗？这只会删除历史备份，不会删除当前任务 checkpoint 和输出文件。");
  if (!ok) return;
  if ($("checkpointActionStatus")) $("checkpointActionStatus").textContent = "正在删除历史任务归档...";
  if ($("taskConsoleStatus")) $("taskConsoleStatus").textContent = "正在删除历史任务归档...";
  const data = await api("/api/delete-checkpoint-archive", {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify({archive_path: path})
  });
  const message = `已删除历史归档：${data.task_name || data.project_config || path}`;
  if ($("checkpointActionStatus")) $("checkpointActionStatus").textContent = message;
  if ($("taskConsoleStatus")) $("taskConsoleStatus").textContent = message;
  await Promise.all([loadProjects(), loadProgress(), loadCheckpoints(), loadStats(), loadTasks()]);
}
...
async function startRuns(containerId, singleIndex = null) {
  ...
  await Promise.all([loadProjects(), loadProgress(), loadStats(), loadTasks()]);
}

async function createNewTask(containerId) {
  const pairs = collectSelectedPairs(containerId);
  const statusEl = containerId === "overviewProjectPairs" ? $("overviewRunStatus") : $("multiStatus");
  if (!pairs.length) {
    statusEl.textContent = "未选择要新建的任务。";
    return;
  }
  const ok = confirm("确认新建任务吗？当前所选项目的 checkpoint 会先归档保存；不会启动任务。归档后你可以修改配置，再点击“启动勾选任务”。");
  if (!ok) return;
  statusEl.textContent = "正在归档当前 checkpoint...";
  const data = await api("/api/new-task", {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify({
      pairs,
      task_name: (prompt("请输入新任务名称；留空则自动使用任务二、任务三这样的默认名称。", "") || "").trim()
    })
  });
  const archived = data.archived_checkpoints?.length || 0;
  statusEl.textContent = `已新建任务：归档 ${archived} 组 checkpoint。现在可以修改患者、模型或项目配置，改完后点击“启动勾选任务”。`;
  if ($("taskConsoleStatus")) $("taskConsoleStatus").textContent = `已新建 ${data.task_name || "新任务"}：归档 ${archived} 组 checkpoint。现在可以修改配置，改完后点击“启动勾选任务”。`;
  await Promise.all([loadProjects(), loadProgress(), loadStats(), loadCheckpoints(), loadTasks()]);
}
...
async function loadConcurrency() {
  const selectedModel = currentModePairs()[0]?.model_config || state.projectState.model_configs?.[0] || "";
  const data = await api(`/api/concurrency?model_config=${encodeURIComponent(selectedModel)}`);
  ...
  $("benchConcurrency").value = data.rule_concurrency || data.model_concurrency || 1;
  if (state.projectState.model_configs.length) {
    $("benchModelSelect").innerHTML = optionList(state.projectState.model_configs, selectedModel);
  }
...
async function saveConcurrency() {
  ...
  const data = await api("/api/concurrency", {
    ...
    body: JSON.stringify({
      ...
      model_config: $("benchModelSelect").value || state.projectState.model_configs?.[0] || "",
...
function bind() {
  ...
  $("overviewNewTaskBtn").addEventListener("click", () => createNewTask("overviewProjectPairs"));
  ...
  $("newTaskBtn").addEventListener("click", () => createNewTask("projectPairs"));
  $("stopRunsBtn").addEventListener("click", () => stopRuns());
  $("checkpointArchiveList").addEventListener("click", event => {
    const deleteButton = event.target.closest("button[data-delete-archive]");
    if (deleteButton) {
      deleteCheckpointArchive(deleteButton.dataset.deleteArchive);
      return;
    }
    const button = event.target.closest("button[data-restore-archive]");
    if (!button) return;
    restoreCheckpointArchive(button.dataset.restoreArchive);
  });
  if ($("taskConsoleList")) {
    $("taskConsoleList").addEventListener("click", event => {
      const startButton = event.target.closest("button[data-start-current-task]");
      if (startButton) {
        startRuns("overviewProjectPairs");
        return;
      }
      const deleteButton = event.target.closest("button[data-delete-archive]");
      if (deleteButton) {
  ...
  for (const containerId of ["projectPairs", "overviewProjectPairs"]) {
    $(containerId).addEventListener("change", event => {
      const projectSelect = event.target.closest("select[data-pair-project]");
      if (projectSelect) {
        savePairSelection(Number(projectSelect.dataset.pairProject), "project_config", projectSelect.value);
        return;
      }
      const modelSelect = event.target.closest("select[data-pair-model]");
      if (modelSelect) {
        savePairSelection(Number(modelSelect.dataset.pairModel), "model_config", modelSelect.value);
      }
    });
...
async function refreshAll() {
  ...
  await Promise.all([loadProgress(), loadStats(), loadConfigs(), loadFiles("."), loadConcurrency(), loadCheckpoints(), loadTasks()]);
  renderWorkbench();
}
...
async function init() {
  ...
  await refreshAll();
  setInterval(async () => {
    await Promise.all([loadServer(), loadProgress(), loadProjects(), loadStats(), loadCheckpoints(), loadTasks()]);
  }, 5000);
}
