...
def try_structured_judge(patient: PatientRecord, rule: TrialRule) -> dict[str, Any] | None:
    text = rule.rule_text
    ...
    if text.strip() in {"男性", "女性"}:
        ...
        if gender:
            ok = wanted in gender
            return _result(rule, "符合" if ok else "不符合", f"患者性别为{gender}。", "raw_patient_info.gender_name", 0.95)
    ...
    if age_match:
        ...
        if birth:
            age = _age(birth)
            ok = age >= threshold
            return _result(rule, "符合" if ok else "不符合", f"出生日期为{birth}，估算年龄{age}岁，阈值为{threshold}岁。", "raw_patient_info.birth_date", 0.9)
