...
class RuleSignature:
    ...
    def cluster_key(self) -> tuple[Any, ...]:
        ...
        domain_bucket = _domain_bucket(self.domains, risk_bucket, self.evidence_category)
        condition_bucket = (
            "numeric" if self.has_numeric_condition else "qualitative"
        ) if risk_bucket == "complex" and self.evidence_category == "lab" else "mixed"
        exception_bucket = "exception" if risk_bucket == "complex" and self.has_exception else "mixed"
        return (
            self.rule_type,
            self.missing_policy,
...
def rules_fingerprint(rules: list[TrialRule]) -> str:
    payload = [
        {
...
def save_batch_plan(
    ...
    unit_by_id = {unit.canonical_rule_id: unit for unit in units}
    payload = {
        ...
        "config": config,
        "summary": optimization_summary(units, batches, len(rules)),
        "batches": [
            {
                "batch_id": index,
                "unit_ids": [unit.canonical_rule_id for unit in batch],
                "rules": [
                    {
                        "trial_id": unit.representative.trial_id,
                        "standard_no": unit.representative.standard_no,
                        "rule_type": unit.representative.rule_type,
                        "complexity": unit.signature.complexity,
                        "evidence_category": unit.signature.evidence_category,
                    }
                    for unit in batch
                ],
            }
    ...
    if missing:
        raise ValueError(f"Batch plan references unknown units: {missing[:5]}")
...
def load_batch_plan(
    ...
    rules: list[TrialRule],
    units: list[JudgmentUnit],
) -> tuple[list[list[JudgmentUnit]], dict[str, Any]]:
    payload = json.loads(path.read_text(encoding="utf-8-sig"))
    if int(payload.get("version") or 0) != BATCH_PLAN_VERSION:
        raise ValueError(f"固定批次计划版本不兼容：{payload.get('version')}")
    expected_fingerprint = rules_fingerprint(rules)
    actual_fingerprint = str(payload.get("rules_fingerprint") or "")
    if actual_fingerprint != expected_fingerprint:
        raise ValueError("固定批次计划与当前规则文件不匹配，请重新生成 batch plan")
    unit_by_id = {unit.canonical_rule_id: unit for unit in units}
    batches: list[list[JudgmentUnit]] = []
    for raw_batch in payload.get("batches", []):
        unit_ids = raw_batch.get("unit_ids", [])
        if not isinstance(unit_ids, list) or not unit_ids:
            raise ValueError("固定批次计划包含空批次")
        batch = []
        for unit_id in unit_ids:
            unit = unit_by_id.get(str(unit_id))
            if unit is None:
                raise ValueError(f"固定批次计划包含未知判断单元：{unit_id}")
            batch.append(unit)
        batches.append(batch)
    validate_optimization_plan(rules, units, batches)
...
def _domain_bucket(domains: tuple[str, ...], risk_bucket: str, evidence_category: str) -> str:
    if not domains:
        return "general"
    if evidence_category != "lab":
        return "shared"
    return domains[0]
